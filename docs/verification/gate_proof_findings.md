...
| [F-57](#f-57) | The source-file connector builds `file://` + the config url, so a Windows backslash path produced a malformed file URL the reader refused; the ingestion config passes each sample as `Path.as_uri()` | Windows rung, Path B |
...
### F-57
**The source-file connector builds `file://` + the config url; a Windows backslash path makes a malformed file URL the reader refuses, so the ingestion config passes each sample as `Path.as_uri()`.** Path B (`uv run mm demo`, the full build through the PyAirbyte file connector) ran on a Windows runner for the first time when `demo-windows` was rebased onto the F-56 fix (run on PR #184). The connector refused the sample with "Please check File Format and Reader Options are set correctly" (`source_file/source.py`), the url in the message reading `file://d:\a\...\ourairports_airports_...csv`. The mechanism, read from the connector source (`source_file/client.py`, airbyte-source-file 0.3.15): for `provider.storage` local the reader computes `full_url = "file://" + url`, where `url` is the config value with any scheme stripped, and opens it with `smart_open`. On macOS and Linux the config carried `str(csv_path)`, a forward-slash absolute path with no scheme, so `full_url` was the valid `file:///home/.../file.csv`. On Windows `str(csv_path)` is `d:\a\...\file.csv`; `urlparse` reads the `d:` as a scheme, the strip leaves the path unchanged, and `full_url` becomes `file://d:\a\...`, which is not a valid file URI, so `smart_open` refuses it. The fix: the config passes `csv_path.as_uri()`. On Windows that is `file:///D:/a/.../file.csv`, which the connector strips to `/D:/a/.../file.csv` and rebuilds as the valid `file:///D:/a/.../file.csv`; on macOS and Linux the reconstructed `full_url` is byte for byte what the plain path produced, so the change is a no-op off Windows. The connector reaches the file only through `full_url` (`smart_open`), never through a raw-path check, so nothing else reads the config url. The PyAirbyte `WinError 5` on deleting the temp config file that appears beside this is not the cause and is not fatal: `airbyte/_util/temp_files.py` retries the delete five times and then only warns. The class: a local file path handed to a tool that treats it as a URL must be a URL, and `str(path)` is a URL only where the separator is already `/`. The fixed tree's Windows run is quoted in the Arc 7 exit record.
(`src/metricmine/ingest/land_sample.py`, the config url; the `demo-windows` run on PR #184)
